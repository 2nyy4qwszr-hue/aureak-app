...
- [ ] 90-1 : pipeline-entraineurs-4-etapes (**P2 FEATURE** — migration 00166 : enum `coach_prospect_status` (5 valeurs : `identifie`/`info_envoyee`/`en_formation`/`actif`/`perdu`), table `coach_prospects` avec RLS admin-tenant/commercial-own + FK nullable `recommended_by_coach_id` (préparation 90-2), types + API CRUD `coach-prospection.ts`, page pipeline `/prospection/entraineurs` (remplace placeholder 88-1, StatCards + tableau 6 colonnes), fiche détail `[prospectId]`, modale RHF+Zod ajout prospect) `ready-for-dev`
