...
- [x] 86-1 : roles-db-manager-marketeur (**P0 INFRA** — migration 00148 + miroir `UserRole` 8 rôles dans `@aureak/types/enums.ts`) `done` — drift BACKLOG, code déjà sur main
- [x] 86-2 : multi-role-profile-roles-switcher (**P0 FEATURE** — migration 00149 + API `auth/profile-roles.ts` + hooks `useCurrentRole`/`useAvailableRoles` + `@aureak/ui/RoleSwitcher` intégré sidebar) `done` — drift BACKLOG, code déjà sur main
- [x] 86-3 : permissions-granulaires-matrice-admin (**P0 FEATURE** — migrations 00150/00151/00152 + API `auth/section-permissions.ts` (`getEffectivePermissions`, `upsert*`, overrides) + page `/settings/permissions` 2 onglets) `done` — drift BACKLOG, code déjà sur main
- [x] 86-4 : sidebar-dynamique-permissions (**P0 FEATURE** — `buildNavGroups(activeRole, permissions)` dans `lib/admin/nav-config.ts` + `useEffectivePermissions` + `(admin)/_layout.tsx` rendu dynamique + RoleSwitcher) `done` — drift BACKLOG, code déjà sur main

**Dépendances** : 86-1 → 86-2, 86-3 (parallèle possible après 86-1) → 86-4 (consomme 86-2 et 86-3). Ordre dev strict : 86-1 → 86-2 → 86-3 → 86-4.

**Migrations utilisées** : 00148 (86-1), 00149 (86-2), 00150 + 00151 + 00152 fix seed (86-3). 86-4 sans migration.

**Audit 2026-04-22** : Epic 86 entièrement présent dans `main`. Migrations 00148–00152 ✅ · types ✅ · API ✅ (`profile-roles.ts`, `section-permissions.ts`) · hooks ✅ (`useCurrentRole`, `useAvailableRoles`, `useEffectivePermissions`) · `RoleSwitcher` ✅ · page `/settings/permissions` ✅ · sidebar dynamique via `buildNavGroups` ✅. `tsc --noEmit` OK. Cohérent avec Epic 87 marqué `done` qui en dépend.
...
- [x] 87-1 : pages-academie-commerciaux-marketeurs-managers (**P1 FEATURE** — +2 onglets `AcademieNavBar` (COMMERCIAUX, MARKETEURS), composant `PeopleListPage` factorisé, pages `/academie/commerciaux`, `/academie/marketeurs`, `/academie/managers` (remplace stub), API `listProfilesByRole`, aucune migration DB — stub `/profiles/[userId]` pour 87-2) `done`
- [x] 87-2 : fiche-personne-universelle (**P1 FEATURE** — layout unifié `/profiles/[userId]` : hero + tabs Résumé/Activité/Accès sync sur ?tab=, modules role-aware Pipeline/Accès étendus/Contenus/Grade coach, card Actions cycle de vie, onglet Accès = placeholder pour 87-3) `done`
- [x] 87-3 : onglet-acces-fiche-personne (**P1 FEATURE** — 3 sections sur `/profiles/[userId]?tab=acces` : rôles chips + modal ajout via `profile_roles`, matrice permissions × sections avec toggle override via `user_section_overrides`, historique dérivé des timestamps — admin only, aucune migration, 2 fonctions API `*History`) `done`
- [x] 87-4 : invitation-dediee-commercial-manager-marketeur (**P1 FEATURE** — 3 routes `/academie/<role>/new` + form slim partagé `NewPersonForm` (4 champs + toggle invite/fiche), helper API `invitePerson` wrappant `create-user-profile` Edge Function existante, remplace les liens `#invite-...` bricolés en 87-1, aucune migration) `done`
- [x] 87-5 : users-redirect-profiles (**P1 REFACTOR** — remplace `/users/[userId]` (DOM brut 398 lignes) par un `<Redirect>` vers `/profiles/[userId]`, update caller unique `/users/index.tsx:232` pour pointer directement, supprime duplication fiche, XS effort) `done`
...
- [x] 89-1 : recherche-ajout-gardien-scout-terrain (migration 00156 + `searchChildDirectoryByName` + formulaire minimal + garde-fou doublon — PR #49 mergée 2026-04-20) `done`
- [x] 89-2 : note-evaluation-scout-rapide (migration 00157 + table `prospect_scout_evaluations` + enum `scout_observation_context` + StarRating 1-5 + soft-delete — PR #50 mergée 2026-04-20) `done`
- [x] 89-3 : visibilite-donnees-conditionnelle-rgpd (migrations 00158 + 00159 + tables `prospect_access_grants`/`_requests`/`_log` + triggers auto-grant + vue `v_child_directory_rgpd` + RPC + masquage SQL + Edge Fn + `MaskedField` + page admin RGPD — PR #51 mergée 2026-04-20, PR #52 fix migrations) `done`

**Audit 2026-04-22** : Epic 89 entièrement présent sur `main`. Migrations 00153-00159 ✅, commits `4cb9770` (89-1), `7972af8` (89-2), `d138897` (89-3). `tsc --noEmit` OK.
...
Objectif : pipeline de recrutement coachs en 4 étapes (identifié → info envoyée → en formation → actif) + recommandation coach interne.

Ordre d'implémentation recommandé : 90-1 → 90-2

- [x] 90-1 : pipeline-entraineurs-4-etapes (**P2 FEATURE** — migration 00167 [drift : 00166 réservé pris par story 96.1] : enum `coach_prospect_status` (5 valeurs) + table `coach_prospects` avec RLS admin-tenant/commercial-own/coach-recommendeur + FK nullable `recommended_by_coach_id` (préparation 90-2), types + API CRUD `coach-prospection.ts`, page pipeline `/prospection/entraineurs` (remplace placeholder 88-1, StatCards + tableau 6 colonnes + filtres), fiche détail `[prospectId]` (statut + coordonnées + profil + notes + placeholder recommandation), modale RHF+Zod ajout prospect) `done`
- [ ] 90-2 : recommandation-coach-prospect-entraineur (**P2 FEATURE** — aucune migration (colonne `recommended_by_coach_id` déjà créée en 90-1, policy INSERT coach à ajouter via 00167 si manquante), API coach `recommendCoachProspect`/`listMyRecommendations`, enrichissement `listCoachProspects`/`getCoachProspect` avec join profiles, card + modale dashboard coach, badge "Recommandé par" fiche prospect, icône tableau, notif in-app admins, section "Mes recommandations" dashboard coach) `ready-for-dev`

**Dépendances** : Epic 86 done ✅ (rôles + permissions) + Story 88-1 done (placeholder `/prospection/entraineurs/` à remplacer). 90-2 requiert 90-1.

**Migrations réservées** : 00166 (90-1). 90-2 sans migration (sauf 00167 policy INSERT coach si manquante).
