...
- [x] 90-2 : recommandation-coach-prospect-entraineur (**P2 FEATURE** — aucune migration (colonne `recommended_by_coach_id` + policy INSERT coach déjà créées en 90-1), API coach `recommendCoachProspect`/`listMyRecommendations` (`packages/api-client/src/coach/recommendations.ts`), card + modale RHF/Zod dashboard coach + section "Mes recommandations", badge "Recommandé par" + section contexte fiche prospect, icône 🤝 tableau pipeline, notif in-app `inapp_notifications` à tous les admins du tenant — silent fail) `done`
