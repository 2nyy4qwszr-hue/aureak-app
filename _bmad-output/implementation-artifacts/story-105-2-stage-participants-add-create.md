...
9. **Retrait** : bouton "Retirer" sur chaque ligne → `ConfirmDialog` (pattern stages story 69.11) → soft-delete dans `child_stage_participations` (`UPDATE deleted_at = now()`). Migration `00171_csp_soft_delete.sql` ajoute la colonne et remplace la contrainte unique par un index partiel sur les inscriptions actives, autorisant la ré-inscription après retrait.
