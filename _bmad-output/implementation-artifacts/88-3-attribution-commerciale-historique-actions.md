...
- [x] Task 1 — Migration Supabase `00149_create_prospect_actions.sql` (AC: #1, #2, #3, #9, #10)
  - [x] Enum `prospect_action_type` (8 valeurs)
  - [x] Table `prospect_actions` : `id` UUID PK, `club_prospect_id` FK `club_prospects`, `performed_by` FK `auth.users`, `action_type` enum, `description` text, `created_at` timestamptz default NOW()
  - [x] PAS de `updated_at` ni `deleted_at` — table append-only
  - [x] Index sur `prospect_actions(club_prospect_id, created_at DESC)`
  - [x] Index sur `prospect_actions(performed_by)`
  - [x] RLS enable
  - [x] Policy SELECT : admin = tout tenant (via join club_prospects.tenant_id), commercial = actions sur ses prospects (`club_prospects.assigned_commercial_id = auth.uid()`)
  - [x] Policy INSERT : commercial peut insérer si `performed_by = auth.uid()` ET prospect assigné à lui
  - [x] PAS de UPDATE/DELETE policies (append-only)
  - [x] Trigger function `log_prospect_status_change()` : AFTER UPDATE OF status ON `club_prospects` → INSERT dans `prospect_actions` avec `action_type = 'changement_statut'`, `description = 'Statut: {OLD.status} → {NEW.status}'`, `performed_by` = current user
- [x] Task 2 — Types TypeScript (AC: #1, #2)
  - [x] `ProspectActionType` type union dans `enums.ts` + `PROSPECT_ACTION_TYPE_LABELS` + `PROSPECT_ACTION_TYPE_ICONS` maps
  - [x] `ProspectAction` entity dans `entities.ts` avec `performerDisplayName` optionnel
  - [x] `CreateProspectActionParams` : `clubProspectId`, `actionType`, `description?`
  - [x] Exporter dans `index.ts`
- [x] Task 3 — API client (AC: #4)
  - [x] Ajouter dans `aureak/packages/api-client/src/admin/prospection.ts` :
  - [x] `listProspectActions(clubProspectId)` — select + join profiles pour displayName, order by `created_at DESC`
  - [x] `addProspectAction(params)` — insert avec `performed_by = currentUser.id`
  - [x] `listMyActions(filters?: { limit?, offset? })` — select where `performed_by = currentUser.id`, order by `created_at DESC`
  - [x] Snake_case → camelCase mapping explicite
- [x] Task 4 — Composant ProspectTimeline (AC: #5, #6)
  - [x] Créer `aureak/apps/web/app/(admin)/developpement/prospection/clubs/_components/ProspectTimeline.tsx`
  - [x] Timeline verticale : ligne verticale gauche + dots colorés par type d'action
  - [x] Chaque entrée : icône type | nom commercial | description | date relative
  - [x] Couleurs par type via `@aureak/theme` tokens
  - [x] Scroll si > 10 actions
- [x] Task 5 — Intégrer timeline dans fiche prospect (AC: #5)
  - [x] Modifier `/developpement/prospection/clubs/[prospectId]/page.tsx` pour afficher `ProspectTimeline`
  - [x] Section "Historique des actions" sous la section contacts
- [x] Task 6 — Modale ajout action (AC: #7)
  - [x] Composant `AddActionModal` — React Hook Form
  - [x] Select type d'action (enum) + textarea description
  - [x] Bouton dans la fiche prospect en haut : "Ajouter une action"
- [x] Task 7 — Vue "Mes actions" (AC: #8)
  - [x] Section dans la page `/developpement/prospection/clubs` ou composant dédié
  - [x] Pour les commerciaux : card résumé avec nombre d'actions ce mois + dernières 5 actions
  - [x] Pour les admins : filtre par commercial
...
- Numéro de migration : `00149` (après 00148 de la story 88-2).

### Fichiers à créer / modifier
| Fichier | Action | Notes |
|---------|--------|-------|
| `supabase/migrations/00149_create_prospect_actions.sql` | CRÉER | Enum + table + trigger + RLS |
...
- `supabase/migrations/00148_create_club_prospects_pipeline.sql` — migration story 88-2
...
- Migration 00148 — FK `club_prospect_id` vers `club_prospects`
