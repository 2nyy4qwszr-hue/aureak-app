...
- [x] Task 1 — Migration Supabase `00151_create_commercial_resources.sql` (AC: #7)
  - [x] Table `commercial_resources` : `id` UUID PK, `tenant_id` FK, `title` text, `description` text, `resource_type` enum `commercial_resource_type` (powerpoint, flyer, webpage, tarifs), `file_path` text nullable (chemin dans le bucket), `external_url` text nullable, `file_size` bigint nullable, `mime_type` text nullable, `uploaded_by` FK `auth.users` nullable, `created_at`, `updated_at`
  - [x] Enum `commercial_resource_type` : `powerpoint`, `flyer`, `webpage`, `tarifs`
  - [x] RLS enable : admin = CRUD, commercial = SELECT
  - [x] Seed les 4 ressources prédéfinies (sans fichier, titre + description pré-remplis)
  - [x] Trigger `updated_at`
- [x] Task 2 — Supabase Storage bucket (AC: #5, #8)
  - [x] Créer bucket `commercial-resources` via migration ou config
  - [x] Storage policies : admin = INSERT/UPDATE/DELETE, tout utilisateur authentifié du tenant = SELECT
  - [x] Limite taille fichier : 50MB
- [x] Task 3 — Types TypeScript (AC: #7)
  - [x] `CommercialResourceType` type union dans `enums.ts` + `COMMERCIAL_RESOURCE_TYPE_LABELS` + icons map
  - [x] `CommercialResource` entity dans `entities.ts`
  - [x] `UpdateCommercialResourceParams` : `{ title?, description?, externalUrl? }`
  - [x] Exporter dans `index.ts`
- [x] Task 4 — API client (AC: #4, #6)
  - [x] Ajouter dans `aureak/packages/api-client/src/admin/prospection.ts` ou fichier dédié `commercial-resources.ts` :
  - [x] `listCommercialResources()` — select all, order by resource_type
  - [x] `updateCommercialResource(id, params)` — update metadata (admin)
  - [x] `uploadCommercialResourceFile(id, file: File)` — upload vers bucket + update file_path/mime_type/file_size (admin)
  - [x] `getResourceDownloadUrl(id)` — URL signée Supabase Storage (ou external_url pour webpage)
  - [x] Snake_case → camelCase mapping explicite
- [x] Task 5 — UI page Ressources (AC: #1, #2, #3)
  - [x] Créer `/developpement/prospection/ressources/page.tsx` + `index.tsx`
  - [x] Modifier `ProspectionNavBar` : ajouter onglet "RESSOURCES"
  - [x] 4 cards en grille 2x2 (ou responsive)
  - [x] Design card : icône type (document/link/spreadsheet), titre bold, description muted, date "Mis à jour le JJ/MM/AAAA", CTA "Télécharger" ou "Ouvrir le lien"
  - [x] Si pas de fichier encore : affichage "Pas encore disponible" en grisé
- [x] Task 6 — UI upload admin (AC: #4)
  - [x] Bouton "Modifier" visible admin only sur chaque card
  - [x] Modale d'upload : file picker (ou input URL pour webpage)
  - [x] Progress bar pendant l'upload
  - [x] Pour webpage : input URL au lieu du file picker
  - [x] Mise à jour automatique de la card après upload
...
- Numéro de migration : `00151` (après 00150 de story 88-4).

### Fichiers à créer / modifier
| Fichier | Action | Notes |
|---------|--------|-------|
| `supabase/migrations/00151_create_commercial_resources.sql` | CRÉER | Table + enum + RLS + seed + bucket |
