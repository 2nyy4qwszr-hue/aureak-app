'use client'
// Hub Partenariat — widget "Sponsors récents" : 5 derniers sponsors actifs
...
export function PartenariatHubRecentSponsors() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) setItems((data ?? []).filter(s => s.isActive).slice(0, 5))
      } catch (err) {
        if (process.env.NODE_ENV !== 'production') console.error('[PartenariatHubRecentSponsors] error:', err)
      } finally {
  ...
  return (
    <View style={styles.card}>
      <AureakText style={styles.title as TextStyle}>Sponsors récents</AureakText>
      {items.length === 0 ? (
            ...
            >
              ...
              <View style={styles.rowContent}>
                ...
                <AureakText style={styles.subline as TextStyle} numberOfLines={1}>
                  {s.activeChildrenCount} parrainage{s.activeChildrenCount > 1 ? 's' : ''}
                </AureakText>
              </View>
            </Pressable>
          ))}
