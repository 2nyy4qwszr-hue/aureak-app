'use client'
// Page Scouts — Template Académie (stub, pas d'API scouts encore)
    ...
    <View style={s.page}>
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent}>
        ...
        <View style={s.statCardsRow}>
          {([
            { picto: '🔍', label: 'SCOUTS',       value: '—', color: colors.text.dark     },
            { picto: '📋', label: 'RAPPORTS',      value: '—', color: colors.accent.gold   },
            { picto: '⚽', label: 'JOUEURS SUIVIS', value: '—', color: colors.text.muted   },
            { picto: '⭐', label: 'DÉTECTIONS',    value: '—', color: colors.status.warning },
          ] as const).map(card => (
        ...
        <View style={s.tableWrapper}>
          <View style={s.tableHeader}>
            ...
            <AureakText style={[s.thText, { flex: 1.5, minWidth: 100 }] as never}>ZONE</AureakText>
            <AureakText style={[s.thText, { width: 90 }] as never}>RAPPORTS</AureakText>
            <AureakText style={[s.thText, { width: 90 }] as never}>DÉTECTIONS</AureakText>
          </View>
          <View style={s.emptyRow}>
            <AureakText style={s.emptyText as TextStyle}>Bientôt disponible — aucun scout enregistré</AureakText>
          </View>
        </View>
