...
import { getThemeByKey, listCriteriaByTheme } from '@aureak/api-client'
...
export default function ThemeDossierPage() {
  ...
  const renderSection = () => {
    switch (activeTab) {
      ...
      case 'identite':     return <SectionIdentite theme={theme} onUpdate={t => setTheme(t)} />
  ...
  return (
    <div style={S.root}>
      {/* Header */}
      <div style={S.header}>
        <button style={S.backBtn} onClick={() => router.push('/referentiel/themes' as never)}>
          ← Thèmes
        </button>
